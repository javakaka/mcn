...
$().ready(function() 
{
	//alert("page is started to initing");
	...
	$submit.click(function() {
		...
		$.ajax({
			...
			cache: false,
			beforeSend: function() {
				//alert("begin disable submit button !");
				//alert("data ...."+$orderForm.serialize());
				$submit.prop("disabled", true);
				//alert("already disable submit button !");
			},
				...
				<form id="orderForm" action="create.jhtml" method="post">
					...
					<div class="span24">
						<div class="bottom">
							...
							<!--
							<a href="javascript:void(0);" onclick="submitOrderForm()" id="submit" class="submit">${message("shop.order.submit")}</a>
							-->
						</div>
					</div>
				</form>
