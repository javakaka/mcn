...
$().ready(function() {
	...
	//alert(" if order is online and order status is unpaid and partialPayment ,then lock order ,check_payment ");
	[#if order.paymentMethod.method == "online" && (order.paymentStatus == "unpaid" || order.paymentStatus == "partialPayment")]
		...
		setInterval(function() {
			$.ajax({
				...
				success: function(data) {
				//(" check_payment over ");
				//alert("check_payment result :"+data);
					if (data) {
						location.href = "view.jhtml?sn=${order.sn}";
					}
				}
