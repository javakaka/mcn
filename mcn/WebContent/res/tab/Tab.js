			...
			for(ii=0; ii<Tabas.length; ii++)
			{
				if(Tabas[ii].Name ==name)
				{
					...
					else
					{
						...
						{
							this.FrameHandle.src =Tabas[ii].Src;
						}
						else if(navigator.userAgent.indexOf("Safari") > -1)
						{
							this.FrameHandle.location.href =Tabas[ii].Src;
						}
						else
						{
							this.FrameHandle.src =Tabas[ii].Src;
 ...
 function TabObj(iSeq, sTabName, sTabTitle, sTabSrc, abled)
{
	...
	if(!this.abled)
		this.oTab.innerHTML ="<span id='span"+sTabName+"'><font color='#747474' style='font-size: 14px;line-height: 26px;'>"+this.Title+"</font></span>";
	else
		this.oTab.innerHTML ="<span id='span"+sTabName+"'><font color='#0000CC' style='font-size: 14px;line-height: 26px;'>"+this.Title+"</font></span>";
