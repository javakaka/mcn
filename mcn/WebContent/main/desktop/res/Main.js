...
CDrag.Table.Rows.prototype = {
	...
	//初始化信息
		...
		if (wc.window == 0) {
			...
			//wc.reduce.innerHTML = "<img src='<%=GlobalUtil._WEB_PATH%>/images/ico_min_max.gif'>";
		} else {
	...
	//获取相关信息
		...
		if (info.src) 
		{
			wc.content.innerHTML = "<iframe height='170px' src='"+WEB_PATH+info.src+"' scrolling=no frameborder=0 width='100%'/>";
...
CDrag.prototype = {
	...
	//变大变小
		...
		if ((o.window = (o.window == 1 ? 0 : 1))) {
			...
			//o.reduce.innerHTML = "<img src='<%=GlobalUtil._WEB_PATH%>/images/ico_min_max.gif'>";
		} else {
	...
	{
		var wc = this;
		var ss =new Server("getStaffPortletJson");
		var s =null;
		if(ss.send(ss,true))
		{
			if(sConfig =="")
				sConfig =ss.getDefaultParamValue("part_serial");
