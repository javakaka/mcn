...
@Component("frameworkSystemThirdAppGrantRoleService")
public class SystemThirdAppGrantRole extends Service{
...
//		 sql= "select posi_no, posi_name, site_no from sm_position where state=1";
	 ...
	 @SuppressWarnings("unchecked")
	 public DataSet getDeptPostionStaffTreeByBureauNo(String bureau_no) throws JException, SQLException
	 {
		 DataSet tempDs =new DataSet();
		 DataSet treeDataSet = new DataSet();
		 String sql= "select bureau_no, bureau_name from sm_bureau where bureau_no='"+bureau_no+"' ";
		 tempDs =queryDataSet(sql);
		 Row localRow2;
		 for(int i=0; i<tempDs.size(); i++)
		 {
