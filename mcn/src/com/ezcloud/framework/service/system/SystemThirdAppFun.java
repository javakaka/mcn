...
@Component("frameworkSystemTirdAppFunService")
public class SystemThirdAppFun extends Service {
...
//		System.out.println("ids=======>>>"+ids.toString());
		...
		rowNum =update(sql);
		//sm_app_role_fun
		sql ="delete from sm_app_role_fun where fun_id in ("+ids.toString()+")";
		rowNum =update(sql);
		return rowNum;
	}
	...
	public DataSet findAllBySpecifiedFunId(String id)
	{
		DataSet ds =new DataSet();
		String specified_up_id ="";
		String sql ="select * from sm_fun";
		ds =queryDataSet(sql);
		//筛选出制定节点的所有上级节点以及下级节点
		DataSet limitedDataSet =new DataSet();
		Row temp =null;
		for(int i=0;i<ds.size();i++)
		{
			temp =(Row)ds.get(i);
			
			String t_id =temp.getString("fun_id",null);
			String up_id =temp.getString("up_fun_id",null);
			if(! StringUtils.isEmptyOrNull(t_id))
			{
				if(t_id.equals(id))
				{
					limitedDataSet.add(temp);
					specified_up_id=up_id;
					break;
				}
				else
				{
		...
		//筛选出制定节点的所有上级节点
		if(! StringUtils.isEmptyOrNull(specified_up_id))
		{
			getAllUpNodes( specified_up_id, ds, limitedDataSet);
		}
		//筛选出制定节点的所有下级节点
		getAllChildNodes(id, ds, limitedDataSet);
		return limitedDataSet;
	}
	
	public void getAllUpNodes(String specified_up_id,DataSet ds,DataSet limitedDs)
	{
		Row temp =null;
		for(int i=0;i<ds.size();i++)
		{
			...
			String t_id =temp.getString("fun_id",null);
			String u_id =temp.getString("up_fun_id",null);
			if(! StringUtils.isEmptyOrNull(t_id))
			{
				if(t_id.equals(specified_up_id))
				{
					limitedDs.add(temp);
					specified_up_id =u_id;
				}
				//continue
				if(! StringUtils.isEmptyOrNull(specified_up_id))
				{
					getAllUpNodes( specified_up_id, ds, limitedDs);
				}
				else
				{
					return;
	...
	public void getAllChildNodes(String specified_id,DataSet ds,DataSet limitedDs)
	{
		Row temp =null;
		for(int i=0;i<ds.size();i++)
		{
			...
			if(! StringUtils.isEmptyOrNull(t_id))
			{
				if(t_id.equals(specified_id))
				{
					continue;
				}
				if(! StringUtils.isEmptyOrNull(up_id))
				{
					if(up_id.equals(specified_id))
					{
						limitedDs.add(temp);
						//continue
						getAllChildNodes( t_id, ds, limitedDs);
					}
				}
			}
		}
...
//		String f_id =null;
	...
	{
		DataSet ds=new DataSet();
//		String sql ="select b.* from sm_app_role_fun a left join sm_fun b on a.fun_id=b.fun_id  where a.ROLE_ID='"+id+"' ";
		String sql ="select b.site_no as fun_id,b.up_site_no as up_fun_id,b.site_name as fun_name from sm_app_role_fun a,sm_site b where a.ROLE_ID='"+id+"' and a.fun_id=b.site_no ";
		ds =queryDataSet(sql);
		return ds;
	}
	...
	public int saveRoleFuns(String role_id,String fun_id)
	{
		...
		String sql ="delete from sm_app_role_fun where role_id='"+role_id+"'";
		update(sql);
		Row role_fun =null;
		for(int i=0; i<arr.length; i++)
		{
			...
			rowNum+=insert("sm_app_role_fun", role_fun);
		}
		return rowNum;
	}
